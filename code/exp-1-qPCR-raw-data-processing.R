...
qlist <- 
  list.files(pattern = "VCE-data.csv") %>%
  map(~read.csv(., header = T, stringsAsFactors = F))
#str(qlist)

# import experiment and molecular data
emdat <- read.csv("sample-exp-molc-data.csv", header = T, stringsAsFactors = F) %>%
  as_tibble()
...
for(i in 1:length(qlist)){
  qlist[[i]]$C_ <- as.numeric(qlist[[i]]$C_)
}
...
qdat <-
  ...
  as_tibble %>%
  rename(well = Well, sample = Sample.Name, target = Target.Name, task = Task, reporter = Reporter, quencher = Quencher, cycle = C_, quantity = Quantity, auto_threshold = Automatic.Ct.Threshold, threshold = Ct.Threshold, auto_baseline = Automatic.Baseline, baseline_start = Baseline.Start, baseline_end = Baseline.End, qPCR_notes = qPCRnotes) %>% # rename columns
  select(-c(C_.Mean, C_.SD, Quantity.Mean, Quantity.SD, Comments, HIGHSD, NOAMP, OUTLIERRG, EXPFAIL)) # remove summarizing columns
...
# some standards appear to be prepared incorrectly - previously went through each run individually (see qPCR-run-comments.R for notes)
